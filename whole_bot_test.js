...
bot.once('spawn', ()=> {
			...
			MyState.prototype.onStateEntered = async function() {
				...
				if (chest.firstEmptyContainerSlot() == null) {
					...
					for (const i of [ancient_debris, gold_nugget, quartz]) {
						await chest.deposit(item.id,null,this.bot.inventory.count(item.id)).catch(err => {
	...
	const transitions = [
		...
		new ms.StateTransition({
			...
			onTransition: () => {
				delay.isFinished = false;
				setTimeout(( () => delay.isFinished = true ), 1000);
			},
		}),
...
// 	if (oldBlock.name!=="air" && newBlock.name==="air" && oldBlock.position.distanceTo(bot.entity.position) < 3){
// 		if ( !T.willSearch.lava.some((i)=> i.equals(oldBlock.position)) )
// 			T.willSearch.lava.push(oldBlock.position);
// 		if ( !T.willSearch.ancient_debris.some((i)=> i.equals(oldBlock.position)) )
// 			T.willSearch.ancient_debris.push(oldBlock.position);
// 	}
// })
...
bot.on("chat", (username, message) => {
	...
	switch(message) {
		case "mine":
			bot.chat('Ready!');
			T.mineCenter = v(4,14,-7); T.axis = v(-1,0,0);
